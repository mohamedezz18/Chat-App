...
import { Member, MemberParams } from '../../../Models/Member';
...
export class MemberList implements OnInit {
  ...
  constructor() {
    const storedParams = localStorage.getItem('filters');
    if (storedParams) {
      this.memberParams = JSON.parse(storedParams);
      this.updatedParams = JSON.parse(storedParams);
    }
  }
  ...
  loadMembers() {
    this._MemberService.getMembers(this.memberParams).subscribe({
      next: (result) => {
        this.PaginatedMember.set(result);
      },
    });
  }
